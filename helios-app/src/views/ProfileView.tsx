...
export function ProfileView() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.solar(), api.posts.list({ limit: 100 }), api.spaces.list()]).then(([solarResult, postResult, spaceResult]) => {
      ...
      if (!cancelled) setLoadError((err as Error).message || 'Profile could not load')
    }).finally(() => {
      ...
      <header className="profile-hero">
        ...
        <div className="profile-solar-card">
          ...
          <div>
            ...
            <small>{solar.next_threshold ? `${Math.max(0, solar.next_threshold - solar.total)} to go` : 'Top level reached'}</small>
          </div>
        </div>
      </header>
      {loading && <div className="profile-journey-empty" role="status">Loading…</div>}
      {loadError && (
        <div className="profile-journey-empty" role="alert">
          {loadError}
          <button type="button" className="liquid-glass-btn" onClick={() => window.location.reload()}>Reload</button>
        </div>
      )}
    ...
    <section className="profile-settings">
      ...
      <article>
        ...
        <div className="profile-setting-row">
          <span><strong>Reduce motion</strong><small>Minimize spatial and realtime transitions.</small></span>
          <button type="button" className={'profile-switch' + (reducedMotion ? ' is-active' : '')} onClick={onMotion} aria-pressed={reducedMotion}><i /></button>
        </div>
      </article>
      <article>
        <h3><Download size={15} /> Your Helios data</h3>
        <p>Download account, Projects and social records currently included in your export.</p>
        <button type="button" className="profile-export" onClick={onExport} disabled={exporting}><Download size={13} /> {exporting ? 'Preparing…' : 'Download JSON export'}</button>
      </article>
      <button type="button" className="profile-logout" onClick={onLogout}><LogOut size={14} /> Sign out</button>
    </section>
  )
}
