  ...
  return (
    <div className="profile-page">
      ...
      <main className="profile-content">
        ...
        {tab === 'Settings' && <SettingsTab theme={state.theme} reducedMotion={state.reducedMotion} exporting={exporting} accountKind={state.user.account_kind || ''} adultPlanActive={hasAdultPlan(state.user)} adultPlanExpiresAt={state.user.adult_plan_expires_at} onTheme={theme => dispatch({ type: 'SET_THEME', theme })} onMotion={() => dispatch({ type: 'SET_REDUCED_MOTION', val: !state.reducedMotion })} onExport={() => void downloadData()} onLogout={() => void logout()} onAccountKind={kind => void api.setAccountKind(kind).then(result => { dispatch({ type: 'SET_USER', user: result.user }); if (kind === 'adult' && !result.user.adult_plan_active) dispatch({ type: 'SET_ADULT_PLAN_OPEN', open: true }) }).catch(reason => dispatch({ type: 'PUSH_TOAST', toast: { id: String(Date.now()), message: (reason as Error).message, tone: 'warning' } }))} onSubscribe={() => dispatch({ type: 'SET_ADULT_PLAN_OPEN', open: true })} />}
      </main>
    </div>
  )
}
...
function SettingsTab({ theme, reducedMotion, exporting, accountKind, adultPlanActive, adultPlanExpiresAt, onTheme, onMotion, onExport, onLogout, onAccountKind, onSubscribe }: { theme: string; reducedMotion: boolean; exporting: boolean; accountKind: string; adultPlanActive: boolean; adultPlanExpiresAt?: string | null; onTheme: (theme: 'dark' | 'high-contrast') => void; onMotion: () => void; onExport: () => void; onLogout: () => void; onAccountKind: (kind: 'student' | 'adult') => void; onSubscribe: () => void }) { return <section className="profile-settings"><header><span>ACCOUNT & ACCESSIBILITY</span><h2>Settings</h2></header><article><h3><Users size={15} /> Student or adult</h3><p>Students keep the free school experience. Adults pay ¥{ADULT_PLAN_PRICE_RMB}/month for work functions and more mature content.</p><div className="profile-theme-buttons"><button type="button" className={accountKind === 'student' ? 'is-active' : ''} onClick={() => onAccountKind('student')}>Student</button><button type="button" className={accountKind === 'adult' ? 'is-active' : ''} onClick={() => onAccountKind('adult')}>Adult</button></div>{accountKind === 'adult' && <div className="profile-setting-row" style={{ marginTop: 12 }}><span><strong>{adultPlanActive ? 'Adult Work Plan active' : 'Adult Work Plan required'}</strong><small>{adultPlanActive && adultPlanExpiresAt ? `Renews around ${new Date(adultPlanExpiresAt).toLocaleDateString()}` : `¥${ADULT_PLAN_PRICE_RMB} per month unlocks Workplace, Career, Finance and City Life.`}</small></span>{!adultPlanActive && <button type="button" className="profile-export" onClick={onSubscribe}>Subscribe</button>}</div>}</article><article><h3><Moon size={15} /> Appearance</h3><div className="profile-theme-buttons"><button type="button" className={theme === 'dark' ? 'is-active' : ''} onClick={() => onTheme('dark')}>Dark</button><button type="button" className={theme === 'high-contrast' ? 'is-active' : ''} onClick={() => onTheme('high-contrast')}>High contrast</button></div></article><article><h3><Settings size={15} /> Accessibility</h3><div className="profile-setting-row"><span><strong>Reduce motion</strong><small>Minimize spatial and realtime transitions.</small></span><button type="button" className={'profile-switch' + (reducedMotion ? ' is-active' : '')} onClick={onMotion} aria-pressed={reducedMotion}><i /></button></div></article><article><h3><Download size={15} /> Your Helios data</h3><p>Download account, Projects and social records currently included in your export.</p><button type="button" className="profile-export" onClick={onExport} disabled={exporting}><Download size={13} /> {exporting ? 'Preparing…' : 'Download JSON export'}</button></article><button type="button" className="profile-logout" onClick={onLogout}><LogOut size={14} /> Sign out</button></section> }
