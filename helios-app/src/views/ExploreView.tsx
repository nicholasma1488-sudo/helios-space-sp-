  ...
  return (
    <div className="explore-page">
      ...
      <main className="explore-content">
        ...
        {!loading && !error && tab === 'books' && <ExploreSection title="Books, writing and comics" eyebrow="READ · LEARN · RESPOND"><div className="explore-book-grid">{bookProjects.map(project => <ProjectDiscoveryCard key={project.id} project={project} onOpen={() => void openProject(project)} book />)}{bookProjects.length === 0 && <ExploreInlineEmpty text="No matching books, documents or comics have been shared." />}</div></ExploreSection>}
...
function CreatorRow({ creator, current }: { creator: ExploreResults['creators'][number]; current: boolean }) { return <div><span>{creator.name.slice(0, 1)}</span><div><strong>{creator.name}</strong><small>{creator.handle} · {creator.project_count} Projects</small></div>{!current && <button type="button" onClick={() => void api.follow(creator.id)}><UserPlus size={12} /></button>}</div> }
